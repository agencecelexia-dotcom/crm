-- Une toiture de pente inconnue ne s'enregistre plus comme une surface au sol.
--
-- L'écran envoyait « null » pour un toit plat, et le serveur, faute de pente,
-- enregistrait l'emprise au sol SANS débord ni part de versant : le chiffre
-- gardé n'était pas celui affiché. Désormais « plate » s'envoie 0 (traité comme
-- une pente, coefficient 1, débord compris), et une toiture relevée sur un
-- bâtiment sans pente du tout est refusée (erreur pente_requise).
...
begin
  ...
  if p_type = 'surface' and coalesce(p_source, 'dessin') = 'bati' and v_pente is null then
