-- ============================================================
--  0016 — Signature de l'apporteur (CELEXIA) figée sur chaque contrat
--  + alignement de la fonction de secours sur le NOUVEAU texte d'apport
--    d'affaires (même texte que le modèle front, variables fusionnées en SQL).
-- ============================================================

alter table public.contrats
  add column if not exists apporteur_signature text;  -- PNG base64 signature CELEXIA (snapshot)

-- ---------- Création idempotente du contrat (texte = nouveau modèle) ----------
...
begin
  ...
  -- Signature de l'apporteur (déposée une fois côté admin)
  select valeur into v_sig from public.app_settings where cle = 'apporteur_signature';

  -- Texte du contrat (identique au modèle front, {{DATE_SIGNATURE}} laissé en jeton)
...
Fait à Nogent-sur-Marne, le {{DATE_SIGNATURE}}, en deux exemplaires originaux.
...
-- ---------- Lecture publique d'un contrat (ajout apporteur_signature) ----------
...
-- ---------- Lecture publique de la mission (ajout apporteur_signature) ----------
