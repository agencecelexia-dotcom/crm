-- ============================================================
--  0072 — L'artisan peut relire les PDF de devis qu'il a déposés.
--
--  get_espace_artisan ne renvoyait que des booléens `devis_depose` /
--  `devis_signe_depose` : l'artisan savait qu'un fichier existait mais n'avait
--  aucun moyen de le consulter — l'interface ne lui proposait que
--  « Remplacer le document ». On expose les URLs.
--
--  Définition RÉGÉNÉRÉE depuis la base (pg_get_functiondef) puis patchée, afin
--  de ne rien perdre des couches successives : bloc `stats` (0070),
--  `projets_perdus` (0070), filtre `retire_at` (0061), nettoyage du pipe.
--
--  Pas de risque de fuite : les URLs pointent vers le bucket `devis`, sous un
--  chemin préfixé par le token d'affectation de CET artisan, et la fonction ne
--  renvoie que ses propres affectations.
-- ============================================================
