-- La hauteur d'un mur se mesure désormais au LiDAR, mur par mur.
--
-- La liste blanche de `hauteur_source` n'acceptait que « bati » et « saisie ».
-- Sans cette correction, une hauteur MESURÉE aurait été enregistrée à NULL —
-- exactement le défaut déjà corrigé pour `pente_source` en 0151 : une valeur
-- mesurée devenue indiscernable d'une valeur de la BD TOPO, qui s'écarte de
-- 3,6 à 4,8 m sur un tiers des maisons comparées.
--
-- Note sur `hauteur_m` pour une façade : c'est la hauteur ÉQUIVALENTE, soit
-- la surface du mur divisée par sa longueur. Le serveur calcule longueur ×
-- hauteur ; avec un profil mesuré (pignon, terrain en pente) ou un pignon
-- ajouté à la main, c'est la seule façon que le chiffre enregistré soit
-- celui que l'artisan a vu.

alter table public.metres drop constraint if exists metres_hauteur_source_check;
alter table public.metres add constraint metres_hauteur_source_check
  check (hauteur_source is null or hauteur_source in ('bati', 'saisie', 'lidar'));

comment on column public.metres.hauteur_m is
  'Pour une façade : hauteur équivalente (surface brute / longueur), qui intègre le pignon et le profil mesuré.';
...
begin
  ...
  values (af.projet_id, af.id, v_artisan,
          ...
          case when p_hauteur_source in ('bati', 'saisie', 'lidar') then p_hauteur_source end)
