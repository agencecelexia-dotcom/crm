...
/** Plus près que ça, un autre bâtiment TOUCHE le mur : même ligne à la numérisation près. */
const CONTACT_M = 0.6

/**
 * La longueur d'un mur le long de laquelle un AUTRE bâtiment le touche.
 *
 * Le relevé LiDAR le devinait en lisant la hauteur à 1,25 m hors du mur : un
 * grand débord de toit, un arbre, un auvent le trompaient — au Thor, tous les
 * murs d'une maison isolée passaient pour accolés, et le total des façades
 * tombait à zéro. La BD TOPO trace les bâtiments voisins : un mur est accolé
 * là où le contour d'un autre bâtiment le longe à moins de 60 cm.
 *
 * @param voisins les contours des AUTRES bâtiments (celui du mur exclu).
 */
export function longueurAccolee(mur: { a: Point; b: Point }, voisins: Point[][]): number {
  const L = distance(mur.a, mur.b)
  if (L < 0.3 || !voisins.length) return 0
  const [bx, by] = enMetres(mur.a, mur.b)

  // Les arêtes voisines, en mètres autour du début du mur — seulement celles
  // qui passent à moins de deux mètres de lui.
  const minX = Math.min(0, bx) - 2, maxX = Math.max(0, bx) + 2
  const minY = Math.min(0, by) - 2, maxY = Math.max(0, by) + 2
  const aretes: [number, number, number, number][] = []
  for (const c of voisins) {
    for (let i = 0; i < c.length; i++) {
      const [x1, y1] = enMetres(mur.a, c[i])
      const [x2, y2] = enMetres(mur.a, c[(i + 1) % c.length])
      if (Math.max(x1, x2) < minX || Math.min(x1, x2) > maxX || Math.max(y1, y2) < minY || Math.min(y1, y2) > maxY) continue
      aretes.push([x1, y1, x2, y2])
    }
  }
  if (!aretes.length) return 0
  ...
  for (let j = 0; j < k; j++) {
    ...
    if (aretes.some(([x1, y1, x2, y2]) => distanceSegment(px, py, x1, y1, x2, y2) <= CONTACT_M)) touche++
  }
  return (touche / k) * L
}
